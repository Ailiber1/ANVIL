...
export default function Hero() {
  ...
  // 0: nothing
  // 1: bg zoom
  // 2: "鍛えるのは、" mask slide-up
  // 3: "体だけじゃない" flicker + stamp
  // 4: rest fades in
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-6 max-w-[1100px] mx-auto">
        ...
        {/* LINE 1: 鍛えるのは、 — mask slide-up */}
        <div className="mb-3 md:mb-4">
          {phase >= 2 && (
            <span className="title-mask">
              <span className="title-slide font-[family-name:var(--font-noto)] text-offwhite/80 text-[1.1rem] sm:text-[1.3rem] md:text-[1.5rem] font-light tracking-[0.35em]">
                鍛えるのは、
              </span>
            </span>
          )}
        ...
        <div className="relative mb-5 md:mb-7">
          ...
          {phase >= 3 && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ animationDelay: "0.6s" }}>
              <div className="gold-flash w-[90%] h-[80%] bg-gold/15 blur-2xl" style={{ animationDelay: "0.5s" }} />
            </div>
          )}

          {phase >= 3 && (
            <h1 className="relative inline-block">
              <FlickerTitle text="体だけじゃない" />
              {/* Gold period — drops after flicker completes */}
              <span
                className="stamp-char font-[family-name:var(--font-cormorant)] text-gold text-[3.5rem] sm:text-[4.5rem] md:text-[6rem] lg:text-[8rem] font-light leading-none align-baseline inline-block translate-y-[0.05em]"
          ...
          {phase >= 3 && (
            <div className="flex justify-center mt-4 md:mt-5">
              <div className="hero-underline h-[2px] w-20 md:w-28 bg-gradient-to-r from-gold/60 to-gold/20" style={{ animationDelay: "1.2s" }} />
            </div>
          )}
        </div>

        {/* English accent */}
        <div className={`transition-all duration-800 ${phase >= 4 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
...
/**
 * FlickerTitle — each character flickers in like a neon sign,
 * with random delay, then final characters stamp in with scale impact.
 */
function FlickerTitle({ text }: { text: string }) {
  const chars = text.split("");
  // Pre-compute random delays for each char so they're stable across renders
  const delays = useMemo(
    () => chars.map((_, i) => {
      // First few chars use flicker, last 3 use stamp for impact
      const isStamp = i >= chars.length - 3;
      const baseDelay = isStamp
        ? 0.5 + i * 0.08
        : Math.random() * 0.5 + i * 0.04;
      return { delay: baseDelay, isStamp };
    }),
  ...
  return (
    <span className="font-[family-name:var(--font-noto)] text-[2.8rem] sm:text-[3.5rem] md:text-[5rem] lg:text-[6.5rem] font-black leading-[1.05] tracking-[0.02em] whitespace-nowrap">
      {chars.map((char, i) => (
        <span
          key={i}
          className={`${delays[i].isStamp ? "stamp-char" : "flicker-char"} text-transparent bg-clip-text bg-gradient-to-r from-offwhite via-gold/90 to-offwhite`}
          style={{ animationDelay: `${delays[i].delay}s` }}
        >
          {char}
        </span>
      ))}
