...
export default function Hero() {
  ...
  useEffect(() => {
    // phase 1: image zoom starts (0ms)
    // phase 2: content reveals (600ms)
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      {/* Giant ANVIL watermark — very subtle */}
      <div
      ...
      <div className="relative z-10 text-center px-6 max-w-[1000px] mx-auto">
        ...
        <div className={`hero-reveal ${show ? "hero-visible" : ""}`} style={{ transitionDelay: "0s" }}>
          <div className="flex items-center justify-center gap-4 mb-10 md:mb-12">
            <div className="w-8 md:w-12 h-px bg-gold/60" />
            <p className="font-[family-name:var(--font-cormorant)] text-gold text-[10px] md:text-[12px] tracking-[0.5em] uppercase">
        ...
        <div className={`hero-reveal ${show ? "hero-visible" : ""}`} style={{ transitionDelay: "0.3s" }}>
          <h1 className="font-[family-name:var(--font-noto)] text-[2.2rem] sm:text-[3rem] md:text-[3.8rem] lg:text-[4.5rem] font-bold leading-[1.15] text-offwhite mb-4 md:mb-5">
            <span className="inline-block">鍛えるのは、</span>
            <br className="md:hidden" />
            <span className="inline-block whitespace-nowrap">体だけじゃない。</span>
          </h1>
        </div>

        {/* English accent — italic, elegant */}
        <div className={`hero-reveal ${show ? "hero-visible" : ""}`} style={{ transitionDelay: "0.5s" }}>
          <p className="font-[family-name:var(--font-cormorant)] text-gold/50 text-[14px] md:text-[17px] tracking-[0.2em] italic mb-10 md:mb-12">
            Forge your body. Shape your life.
          </p>
        </div>

        {/* Divider line with expand animation */}
        <div className={`hero-reveal ${show ? "hero-visible" : ""}`} style={{ transitionDelay: "0.7s" }}>
          <div className="w-16 h-px bg-gold/40 mx-auto mb-10 md:mb-12" />
        </div>

        {/* Sub copy */}
        <div className={`hero-reveal ${show ? "hero-visible" : ""}`} style={{ transitionDelay: "0.9s" }}>
          <p className="text-warmgray/80 text-[13px] md:text-[15px] leading-[2.4] mb-12 md:mb-16 max-w-[550px] mx-auto">
            姿勢、自信、習慣、人生。
            <br />
        ...
        {/* CTA buttons */}
        <div className={`hero-reveal ${show ? "hero-visible" : ""}`} style={{ transitionDelay: "1.1s" }}>
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <a
              href="/contact"
              className="btn-shimmer text-obsidian font-medium px-12 py-4 text-[13px] tracking-[0.1em] transition-all duration-500 hover:shadow-[0_0_40px_rgba(201,169,110,0.25)] inline-block"
            >
              無料カウンセリングを予約する
            </a>
            <a
              href="#program"
              className="group border border-offwhite/30 text-offwhite font-medium px-12 py-4 text-[13px] tracking-[0.1em] hover:border-gold hover:text-gold transition-all duration-500 inline-block relative overflow-hidden"
            >
        ...
        {/* LINE link */}
        <div className={`hero-reveal ${show ? "hero-visible" : ""}`} style={{ transitionDelay: "1.3s" }}>
          <a
            href={LINE_URL}
            target="_blank"
            rel="noopener noreferrer"
            aria-label="LINEで予約する"
            className="inline-flex items-center gap-2 text-warmgray/60 text-[10px] tracking-[0.08em] hover:text-line-green transition-colors duration-300"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-line-green/60">
